...
void idPlayer::UpdateHudStats( idUserInterface *_hud ) {
	...
	temp1 = _hud->State().GetString("player_class", "Base");
	if (temp1 != inventory.characterClass) {
		_hud->SetStateString("player_class", inventory.characterClass);
		_hud->HandleNamedEvent("updateClass");
	}
...
// RAVEN END
	...
	if ( damage > 0 ) {
		...
		damage = int(damage * (1 - (((inventory.defense / 3) + (inventory.resistance / 3)) / 100)));
		health -= damage;
